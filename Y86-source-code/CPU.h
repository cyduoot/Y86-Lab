...
map <int, int> mem;
string F_op, D_op, E_op, M_op, W_op;
...
//		head++;
	...
	void Decode(){
		...
		if (d_srcA != RNONE)
			D_op = D_op + "d_srcA <- " + int2str(d_srcA) + '\n';
		...
		if (d_srcB != RNONE)
			D_op = D_op + "d_srcB <- " + int2str(d_srcB) + '\n';
		...
		if (d_dstE != RNONE)
			D_op = D_op + "d_dstE <- " + int2str(d_dstE) + '\n';
		
		if (D_icode == IMRMOVL || D_icode == IPOPL) d_dstM = D_rA;
		else d_dstM = RNONE;
		if (d_dstM != RNONE)
			D_op = D_op + "d_dstM <- " + int2str(d_dstM) + '\n';
	...
	void Execute(){
		...
		if (e_dstE != RNONE)
			E_op = E_op + "e_dstE <- " + int2str(e_dstE) + '\n';
